...
import { firstRow, parseCountField, numberField, validateBehavioralProfile, validateContractProfile } from '../db-driver/result';
...
export class HomologInferenceEngine {
            ...
            for (const hom of homologs) {
                ...
                await db.queryWithClient(client, `
                    ...
                    DO UPDATE SET confidence = GREATEST(inferred_relations.confidence, EXCLUDED.confidence),
                                  evidence_bundle_id = CASE
                                      WHEN EXCLUDED.confidence >= inferred_relations.confidence
                                      THEN EXCLUDED.evidence_bundle_id
                                      ELSE inferred_relations.evidence_bundle_id
                                  END
                `, [
    ...
    private async generateCandidates(
        ...
        try {
            ...
            const newSvIds = semanticCandidates
                .map(sc => sc.svId)
                .filter(svId => !candidateMap.has(svId));

            if (newSvIds.length > 0) {
                const placeholders = newSvIds.map((_, i) => `$${i + 1}`).join(',');
                ...
                `, newSvIds);
                for (const row of batchResult.rows as CandidateRow[]) {
                    candidateMap.set(row.symbol_version_id, row);
                }
            }
        } catch (error) {
            log.debug('Semantic candidate generation skipped (no vectors)', {
                snapshotId,
    ...
    private async scoreCandidate(
        ...
        try {
            ...
            // Guard against NaN/Infinity from cosine similarity edge cases
            if (!Number.isFinite(semanticSim)) semanticSim = 0;
        } catch (error) {
            // Fallback to name-based Jaccard if semantic vectors unavailable
            log.debug('Semantic similarity fallback engaged', {
                target_symbol_version_id: target.symbol_version_id,
                candidate_symbol_version_id: candidate.symbol_version_id,
                error: error instanceof Error ? error.message : String(error),
            });
        ...
        if (target.body_hash === candidate.body_hash) {
            ...
            try {
                ...
                log.debug('Body similarity unavailable; using zero fallback', {
                    target_symbol_version_id: target.symbol_version_id,
                    candidate_symbol_version_id: candidate.symbol_version_id,
                    error: error instanceof Error ? error.message : String(error),
                });
        ...
        const safeSemantic = Number.isFinite(semanticSim) ? semanticSim : 0;
        const safeLogic = Number.isFinite(logicSim) ? logicSim : 0;
        const safeSig = Number.isFinite(sigSim) ? sigSim : 0;
        const safeBehavioral = Number.isFinite(behavioralOverlap) ? behavioralOverlap : 0;
        const safeContract = Number.isFinite(contractOverlap) ? contractOverlap : 0;
        const safeTest = Number.isFinite(testOverlap) ? testOverlap : 0;
        const safeHistory = Number.isFinite(historySim) ? historySim : 0;

        // Weighted total
        let weightedTotal =
            HOMOLOG_WEIGHTS.semantic_intent_similarity * safeSemantic +
            HOMOLOG_WEIGHTS.normalized_logic_similarity * safeLogic +
            HOMOLOG_WEIGHTS.signature_type_similarity * safeSig +
            HOMOLOG_WEIGHTS.behavioral_overlap * safeBehavioral +
            HOMOLOG_WEIGHTS.contract_overlap * safeContract +
            HOMOLOG_WEIGHTS.test_overlap * safeTest +
            HOMOLOG_WEIGHTS.history_co_change * safeHistory;
        ...
        return {
            semantic_intent_similarity: safeSemantic,
            normalized_logic_similarity: safeLogic,
            signature_type_similarity: safeSig,
            behavioral_overlap: safeBehavioral,
            contract_overlap: safeContract,
            test_overlap: safeTest,
            history_co_change: safeHistory,
            weighted_total: Math.min(1.0, Math.max(0, weightedTotal)),
            evidence_family_count: familyCount,
            rationale: this.buildRationale(safeSemantic, safeLogic, safeSig,
                safeBehavioral, safeContract, safeTest, safeHistory),
        };
    }
    ...
    private async detectContradictions(
        ...
        if (targetBp && candidateBp) {
            ...
            const sortedTargetAuth = [...targetAuth].sort();
            const sortedCandidateAuth = [...candidateAuth].sort();
            if (sortedTargetAuth.length !== sortedCandidateAuth.length ||
                !sortedTargetAuth.every((v, i) => v === sortedCandidateAuth[i])) {
    ...
    private async computeBehavioralOverlap(svIdA: string, svIdB: string): Promise<number> {
        ...
        if (setA.size === 0 && setB.size === 0) {
            // Both have zero resource touches.
            // If both are 'pure' — this is the default state for most symbols,
            // carries no homolog signal. Return 0 to avoid inflating scores
            // for unrelated code.
            if (a.purity_class === 'pure' && b.purity_class === 'pure') {
                return 0.0;
            }
            // If both are non-pure with same purity class but no specific resources,
            // they share a meaningful behavioral classification. Give partial credit
            // to seed concept family clustering.
            if (a.purity_class === b.purity_class) {
                return 0.3;
            }
            // Different non-pure purity classes with no resources — no signal
            return 0.0;
        }
    ...
    private async loadBehavioralProfile(svId: string): Promise<BehavioralProfile | null> {
        ...
        const profile = result.rows[0]
            ? validateBehavioralProfile(result.rows[0] as Record<string, unknown>)
            : null;
    ...
    private async loadContractProfile(svId: string): Promise<ContractProfile | null> {
        ...
        const profile = result.rows[0]
            ? validateContractProfile(result.rows[0] as Record<string, unknown>)
            : null;
        if (profile) profileCache.set(cacheKey, profile);
        return profile;
    }
