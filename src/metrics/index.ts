...
/** Safety cap to prevent unbounded memory growth from label cardinality explosion */
...
export function incrementCounter(name: string, labels?: Record<string, string>): void {
    ...
    if (existing) {
        ...
        if (counters.size >= MAX_METRIC_SERIES) {
            if (!capWarningEmitted) { capWarningEmitted = true; process.stderr.write(JSON.stringify({ timestamp: new Date().toISOString(), level: 'warn', subsystem: 'metrics', message: `Metric series cap (${MAX_METRIC_SERIES}) reached — new series are being dropped. Possible label cardinality explosion.` }) + '\n'); }
            return;
        }
...
export function observeHistogram(name: string, value: number, labels?: Record<string, string>): void {
    if (!Number.isFinite(value)) return; // Guard against NaN/Infinity corrupting sum
    ...
    if (!data) {
        if (histograms.size >= MAX_METRIC_SERIES) {
            if (!capWarningEmitted) { capWarningEmitted = true; process.stderr.write(JSON.stringify({ timestamp: new Date().toISOString(), level: 'warn', subsystem: 'metrics', message: `Metric series cap (${MAX_METRIC_SERIES}) reached — new series are being dropped. Possible label cardinality explosion.` }) + '\n'); }
            return;
        }
        data = {
    ...
    data.sum += value;
    data.count++;
    // Note: bucketCounts stores per-bucket (non-cumulative) counts.
    // The renderMetrics function accumulates them into cumulative Prometheus format.
    ...
    // Values exceeding all bucket boundaries are NOT placed in any defined bucket.
    // They are captured only by sum/count (already incremented above) and appear
    // in the +Inf bucket at render time via data.count (see renderHistograms).
}
...
export function setGauge(name: string, value: number): void {
    if (!gauges.has(name) && gauges.size >= MAX_METRIC_SERIES) {
        if (!capWarningEmitted) { capWarningEmitted = true; process.stderr.write(JSON.stringify({ timestamp: new Date().toISOString(), level: 'warn', subsystem: 'metrics', message: `Metric series cap (${MAX_METRIC_SERIES}) reached — new series are being dropped. Possible label cardinality explosion.` }) + '\n'); }
        return;
    }
...
const SAFE_PATH_LABEL_RE = /^\/(?:health|ready|metrics|scg_[a-z0-9_]+)$/;

function getMetricPathLabel(req: Request): string {
    const routePath = req.route?.path;
    if (typeof routePath === 'string' && routePath.length > 0) {
        return routePath;
    }
    if (typeof req.path === 'string' && SAFE_PATH_LABEL_RE.test(req.path)) {
        return req.path;
    }
    return 'unmatched';
}
...
export function metricsMiddleware(req: Request, res: Response, next: NextFunction): void {
    ...
    res.on('finish', () => {
        ...
        // Use matched route pattern (e.g., "/scg_resolve_symbol"), NOT the raw URL path.
        // Raw paths include dynamic segments (UUIDs, etc.) which would create unbounded
        // label cardinality — a memory leak that grows with every unique request URL.
        const labels = {
            method: req.method,
            path: getMetricPathLabel(req),
        ...
        if (res.locals?.['authFailure'] === true || res.statusCode === 401 || res.statusCode === 403) {
