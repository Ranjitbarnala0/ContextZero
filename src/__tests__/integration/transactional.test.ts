...
// Mock fs so rollback's realpathSync doesn't fail on non-existent test paths
jest.mock('fs', () => {
...
// Mock fs/promises for async file operations used in rollback/commit
...
describe('Transactional Integration — Engine State Machine', () => {
    ...
    beforeEach(() => {
        ...
        // Make mockTransaction execute its callback with a mock client
        // so transitionState (which now uses db.transaction) works correctly
        mockTransaction.mockImplementation(async (cb: (client: { query: typeof mockQueryWithClient }) => Promise<void>) => {
            return cb({ query: mockQueryWithClient });
        });
    });
    ...
    test('commit transitions from validated to committed', async () => {
        // New commit() runs entirely inside db.transaction with queryWithClient:
        // 1. SELECT state FOR UPDATE
        mockQueryWithClient.mockResolvedValueOnce({
            rows: [{ state: 'validated' }],
            rowCount: 1,
        });
        // 2. UPDATE state to committed
    ...
    test('commit rejects from planned state', async () => {
        // New commit() uses db.transaction with FOR UPDATE — mock the locked row
        mockQueryWithClient.mockResolvedValueOnce({
            rows: [{ state: 'planned' }],
            rowCount: 1,
        });
    ...
    test('rollback restores file backups and transitions to rolled_back', async () => {
        ...
        // SELECT backups (no backups)
        mockQuery.mockResolvedValueOnce({
            rows: [],
            rowCount: 0,
        });
        // transitionState now runs inside db.transaction — mock client queries:
    ...
    test('rollback rejects from committed state', async () => {
        ...
        // New rollback validates state BEFORE any file operations,
        // so it throws immediately without needing getRepoBasePath or backup mocks
