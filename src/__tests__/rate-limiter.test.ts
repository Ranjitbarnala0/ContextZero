...
describe('Rate Limiter', () => {
    ...
    test('includes correlation id in rejected responses when available', () => {
        for (let i = 0; i < 60; i++) {
            const { req, res, next } = createMock('/test');
            rateLimitMiddleware(req, res, next);
        }
        ...
        const mock = createMock('/test');
        mock.req.correlationId = 'req-rate-limit';
        rateLimitMiddleware(mock.req, mock.res, mock.next);

        expect(mock.getStatus()).toBe(429);
        expect(mock.getBody()).toEqual(expect.objectContaining({ correlationId: 'req-rate-limit' }));
    });
    ...
    test('refills capacity over time', () => {
        ...
        const blocked = createMock('/test');
        rateLimitMiddleware(blocked.req, blocked.res, blocked.next);
        expect(blocked.getStatus()).toBe(429);

        jest.advanceTimersByTime(1_000);

        const allowedAgain = createMock('/test');
        rateLimitMiddleware(allowedAgain.req, allowedAgain.res, allowedAgain.next);
        expect(allowedAgain.next).toHaveBeenCalled();
    });
});
