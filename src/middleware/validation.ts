...
export function isValidUUIDArray(value: unknown): value is string[] {
    return Array.isArray(value) && value.length > 0 && value.length <= 20 && value.every(isValidUUID);
}
...
export const requireUUIDArray = (value: unknown): string | null => {
    if (value === undefined || value === null) return 'required';
    if (!isValidUUIDArray(value)) return 'must be an array of 1-20 valid UUIDs';
    return null;
};
...
export const optionalBoundedInt = (min: number, max: number) => (value: unknown): string | null => {
    if (value === undefined || value === null) return null;
    if (!isBoundedNumber(value, min, max)) return `must be a number between ${min} and ${max}`;
    return null;
};

/** Validates a required bounded integer — rejects undefined/null and out-of-range */
...
export const requirePatchArray = (value: unknown): string | null => {
    ...
    for (let i = 0; i < value.length; i++) {
        ...
        // Reject null bytes — truncate paths in C-based functions
        if (filePath.includes('\0')) {
            return `patches[${i}].file_path: null bytes not allowed`;
        }
