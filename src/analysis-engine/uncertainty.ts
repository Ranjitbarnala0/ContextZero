...
import { firstRow, parseCountField } from '../db-driver/result';
...
export class UncertaintyTracker {
    ...
    public computeSymbolConfidence(uncertaintyFlags: string[]): number {
        ...
        // Deduplicate flags to prevent double-counting the same uncertainty source
        const uniqueFlags = [...new Set(uncertaintyFlags)];
        let confidence = 1.0;
    ...
    public async getSnapshotUncertainty(snapshotId: string): Promise<{
        ...
        const total = parseCountField(firstRow(totalSymbols));
        const profiled = parseCountField(firstRow(profiledSymbols));
        ...
        if (total > 0 && profiled < total * 0.5) {
            ...
            const uncoveredFraction = 1 - (profiled / total);
            bySource['incomplete_behavioral_coverage'] = total - profiled;
            totalAnnotations += 1;
            allAnnotations.push({
                source: 'untested_path',
                affected_symbol_id: null,
                description: `Only ${coverage}% of symbols have behavioral profiles (${profiled}/${total})`,
                confidence_impact: Math.min(0.50, uncoveredFraction * 0.50),
                recommended_evidence: this.recommendEvidence('untested_path'),
            });
        }
        ...
        const tested = parseCountField(firstRow(testedSymbols));
        if (total > 0 && tested === 0) {
